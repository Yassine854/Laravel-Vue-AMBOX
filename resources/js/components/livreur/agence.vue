    ...
    <layout_livreur ref="table">
      ...
      >
        ...
        <div class="card-header d-flex align-items-center">
            <i class="fa-solid fa-earth-oceania me-2"></i>
        ...
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <template v-if="displayedAgenceSlice.length > 0">
              <tbody>
                <tr
                  v-for="agence in displayedAgenceSlice"
                  :key="agence.id"
                  @click="showColis(agence.agence)"
                  id="crudBtn"
                >
...
import tunisia from "../../../../storage/app/public/json/zip-postcodes.json";
  ...
  export default {
    ...
    this.get_livreurs_colis(this.user);
  ...
  computed: {
    displayedAgence() {
      ...
      return this.agences.filter((agence) => {
        const nom = (agence.agence || "").toLowerCase();
    ...
    displayedPages() {
      const pageCount = Math.ceil(
        this.displayedAgence.length / this.itemsPerPage
      );
      const range = [];
    ...
    displayedAgenceSlice() {
      const startIndex = (this.currentPage - 1) * this.itemsPerPage;
      const endIndex = startIndex + this.itemsPerPage;
      return this.displayedAgence.slice(startIndex, endIndex);
    },
  ...
  methods: {
    ...
    async get_livreurs_colis(user) {
        console.log(user.id);
      try {
        const response = await axios.get(`/api/colis/get_livreurs_colis/${user.id}`);
        this.agences = response.data.colis;
    ...
    showColis(agence_name) {
        console.log(agence_name);
            this.$router.push({
                name: 'colis_livreur',
                params: { agence: agence_name },
                props: true
         });
      },
