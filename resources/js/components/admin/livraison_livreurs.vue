<template>
    ...
    <div class="d-flex align-items-center">
        ...
        <h5 class="mb-0">Livreur : {{ livreur_name }}</h5>
    </div>
...
import tunisia from "../../../../storage/app/public/json/zip-postcodes.json";
  ...
  methods: {
    ...
    async get_all_livraisons() {
      try {
        const response = await axios.get(`/api/livraisons/get_all_livraisons/${this.livreur_id}`);
    ...
    async get_livreur_name(){
        try {
        const response=  await axios.get(`/api/livreurs/get_livreur_name/${this.livreur_id}`);
          this.livreur_name=response.data.livreur_name;
