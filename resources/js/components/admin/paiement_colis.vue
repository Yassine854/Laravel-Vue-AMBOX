    ...
    <layout_admin ref="table">
      ...
      >
        ...
        <div class="card-header d-flex align-items-center">
          ...
          <h5 class="mb-0">Paiement des colis</h5>
        </div>
            ...
            <template v-if="displayedColisSlice.length > 0">
              <tbody>
                ...
                >
                  ...
                  <td>{{ coli.nom }}</td>
                  <td>{{ coli.prix }} Dt</td>
                  <td v-if="coli.payer ==0">Non</td>
                  <td v-if="coli.payer ==1">Oui</td>
                  <td>
...
import tunisia from "../../../../storage/app/public/json/zip-postcodes.json";
  ...
  methods: {
    ...
    async get_all_paiements_colis() {
      try {
        const response = await axios.get(`/api/colis/get_all_paiements_colis`);
...
doc.autoTable({
  ...
  columnStyles: {
    ...
    // Montant
    doc.autoTable({
  head: [['Montant HT:','Prix de livraison','TVA 19%:']],
  body: [[Math.round(((this.colisInfo.prix/(1+19/100))-(this.colisInfo.expediteur.prix_livraison))*100)/100+ " Dt",
  this.colisInfo.expediteur.prix_livraison+ " Dt",
